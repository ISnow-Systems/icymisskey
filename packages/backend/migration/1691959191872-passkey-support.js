...
export class PasskeySupport1691959191872 {
	...
	async up(queryRunner) {
		await queryRunner.query(`ALTER TABLE "user_security_key"
			ADD "counter" bigint NOT NULL DEFAULT '0'`);
		...
		await queryRunner.query(`ALTER TABLE "user_security_key"
			ADD "credentialDeviceType" character varying(32)`);
		...
		await queryRunner.query(`ALTER TABLE "user_security_key"
			ADD "credentialBackedUp" boolean`);
		...
		await queryRunner.query(`ALTER TABLE "user_security_key"
			ADD "transports" character varying(32) array`);
		...
		await queryRunner.query(`ALTER TABLE "user_security_key"
			ALTER COLUMN "lastUsed" SET DEFAULT now()`);
		await queryRunner.query(`UPDATE "user_security_key"
								 SET "id"        = REPLACE(REPLACE(REPLACE(REPLACE(ENCODE(DECODE("id", 'hex'), 'base64'), E '\\n', ''), '+', '-'), '/', '_'), '=', ''),
									 "publicKey" = REPLACE(REPLACE(REPLACE(REPLACE(ENCODE(DECODE("publicKey", 'hex'), 'base64'), E '\\n', ''), '+', '-'), '/', '_'), '=', '')`);
	...
	async down(queryRunner) {
		await queryRunner.query(`CREATE TABLE "attestation_challenge"
								 (
									 ...
									 "registrationChallenge" boolean                  NOT NULL DEFAULT false,
									 CONSTRAINT "PK_d0ba6786e093f1bcb497572a6b5" PRIMARY KEY ("id", "userId")
								 )`);
		...
		await queryRunner.query(`ALTER TABLE "attestation_challenge"
			ADD CONSTRAINT "FK_f1a461a618fa1755692d0e0d592" FOREIGN KEY ("userId") REFERENCES "user" ("id") ON DELETE CASCADE ON UPDATE NO ACTION`);
		...
		await queryRunner.query(`UPDATE "user_security_key"
								 SET "id"        = ENCODE(DECODE(REPLACE(REPLACE("id" || CASE WHEN LENGTH("id") % 4 = 2 THEN '==' WHEN LENGTH("id") % 4 = 3 THEN '=' ELSE '' END, '-', '+'), '_', '/'), 'base64'), 'hex'),
									 "publicKey" = ENCODE(DECODE(REPLACE(REPLACE("publicKey" || CASE WHEN LENGTH("publicKey") % 4 = 2 THEN '==' WHEN LENGTH("publicKey") % 4 = 3 THEN '=' ELSE '' END, '-', '+'), '_', '/'), 'base64'), 'hex')`);
		await queryRunner.query(`ALTER TABLE "user_security_key"
