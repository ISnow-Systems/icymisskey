				...
				<div class="_gaps">
					<MkFolder>
						...
						<div class="_gaps_s">
							...
							<MkFolder v-if="matchQuery([i18n.ts._role._options.rateLimitFactor, 'rateLimitFactor'])">
								...
								<MkRange :max="300" :min="30" :modelValue="policies.rateLimitFactor * 100" :step="10" :textConverter="(v) => `${v}%`" @update:modelValue="v => policies.rateLimitFactor = (v / 100)">
							...
							<MkFolder v-if="matchQuery([i18n.ts._role._options.avatarDecorationLimit, 'avatarDecorationLimit'])">
								...
								<MkInput v-model="avatarDecorationLimit" :max="16" :min="0" type="number" @update:modelValue="updateAvatarDecorationLimit">
								</MkInput>
							</MkFolder>
					...
					<div class="_gaps_s">
						<MkFoldableSection>
							...
							<div class="_gaps_s">
								<MkRolePreview v-for="role in roles.filter(x => x.target === 'manual')" :key="role.id" :forModeration="true" :role="role"/>
							</div>
						</MkFoldableSection>
						<MkFoldableSection>
							<template #header>{{ i18n.ts._role.conditionalRoles }}</template>
							<div class="_gaps_s">
								<MkRolePreview v-for="role in roles.filter(x => x.target === 'conditional')" :key="role.id" :forModeration="true" :role="role"/>
							</div>
						</MkFoldableSection>
					</div>
				</div>
