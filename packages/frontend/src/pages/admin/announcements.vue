			...
			<div class="_gaps">
				...
				<template v-else>
					<MkFolder v-for="announcement in announcements" :key="announcement.id ?? announcement._id" :defaultOpen="announcement.id == null">
						...
						<template #footer>
							<div class="_buttons">
								<MkButton primary rounded @click="save(announcement)"><i class="ti ti-device-floppy"></i> {{ i18n.ts.save }}</MkButton>
								...
								<MkButton v-if="announcement.id != null" danger rounded @click="del(announcement)"><i class="ti ti-trash"></i> {{ i18n.ts.delete }}</MkButton>
							</div>
						</template>
						...
						<div class="_gaps">
							...
							<MkTextarea v-model="announcement.text" :mfmPreview="true" mfmAutocomplete>
