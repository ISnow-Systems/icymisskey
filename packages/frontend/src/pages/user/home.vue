...
<template>
	<MkSpacer :contentMax="narrow ? 800 : 1100">
		<div ref="rootEl" :class="{ wide: !narrow }" class="ftskorzw" style="container-type: inline-size;">
			<div class="main _gaps">
				...
				<div class="profile _gaps">
					...
					<div :key="user.id" class="main _panel">
						<div :style="style" class="banner-container">
							<div ref="bannerEl" :style="style" class="banner"></div>
							<div class="fade"></div>
							<div class="title">
								<MkUserName :nowrap="true" :user="user" class="name"/>
								<div class="bottom">
									<span class="username"><MkAcct :detail="true" :user="user"/></span>
							...
							<div class="actions">
								...
								<MkFollowButton v-if="$i?.id != user.id" v-model:user="user" :full="true" :inline="true" :transparent="false" class="koudoku"/>
							</div>
						</div>
						...
						<div v-if="user.followedMessage != null" class="followedMessage">
							<MkFukidashi :tail="narrow ? 'none' : 'left'" class="fukidashi" negativeMargin shadow>
								<div class="messageHeader">{{ i18n.ts.messageToFollower }}</div>
								<div>
									<MkSparkle>
										<Mfm :author="user" :plain="true" :text="user.followedMessage"/>
									</MkSparkle>
								</div>
							</MkFukidashi>
						</div>
						<div v-if="user.roles.length > 0" class="roles">
						<span v-for="role in user.roles" :key="role.id" v-tooltip="role.description" :style="{ '--color': role.color }" class="role">
							<MkA v-adaptive-bg :to="`/roles/${role.id}`">
								<img v-if="role.iconUrl" :src="role.iconUrl" style="height: 1.3em; vertical-align: -22%;"/>
								{{ role.name }}
							</MkA>
						</span>
						</div>
						...
						<div v-if="isEditingMemo || memoDraft" :class="{'no-memo': !memoDraft}" class="memo">
						...
						<div class="description">
							<MkOmit>
								<Mfm v-if="user.description" :author="user" :isNote="false" :text="user.description"/>
						...
						<div v-if="user.fields.length > 0" class="fields">
							<dl v-for="(field, i) in user.fields" :key="i" class="field">
								<dt class="name">
									<Mfm :author="user" :colored="false" :plain="true" :text="field.name"/>
								</dt>
								<dd class="value">
									<Mfm :author="user" :colored="false" :text="field.value"/>
									<i v-if="user.verifiedLinks.includes(field.value)" v-tooltip:dialog="i18n.ts.verifiedLink" :class="$style.verifiedLink" class="ti ti-circle-check"></i>
								</dd>
							</dl>
						</div>
						<div class="status">
				...
				<div class="contents _gaps">
					<div v-if="user.pinnedNotes.length > 0" class="_gaps">
						<MkNote v-for="note in user.pinnedNotes" :key="note.id" :note="note" :pinned="true" class="note _panel"/>
					</div>
