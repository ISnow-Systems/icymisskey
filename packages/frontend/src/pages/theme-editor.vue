			...
			<div class="cwepdizn _gaps_m">
				<MkFolder :defaultOpen="true">
					...
					<div class="cwepdizn-colors">
						<div class="row">
							<button v-for="color in bgColors.filter(x => x.kind === 'light')" :key="color.color" :class="{ active: theme.props.bg === color.color }" class="color _button" @click="setBgColor(color)">
								<div :style="{ background: color.forPreview }" class="preview"></div>
							</button>
						</div>
						<div class="row">
							<button v-for="color in bgColors.filter(x => x.kind === 'dark')" :key="color.color" :class="{ active: theme.props.bg === color.color }" class="color _button" @click="setBgColor(color)">
								<div :style="{ background: color.forPreview }" class="preview"></div>
							</button>
						</div>
					</div>
				...
				<MkFolder :defaultOpen="true">
					...
					<div class="cwepdizn-colors">
						<div class="row">
							<button v-for="color in accentColors" :key="color" :class="{ active: theme.props.accent === color }" class="color rounded _button" @click="setAccentColor(color)">
				...
				<MkFolder :defaultOpen="true">
					...
					<div class="cwepdizn-colors">
						<div class="row">
							<button v-for="color in fgColors" :key="color" :class="{ active: (theme.props.fg === color.forLight) || (theme.props.fg === color.forDark) }" class="color char _button" @click="setFgColor(color)">
								<div :style="{ color: color.forPreview ? color.forPreview : theme.base === 'light' ? '#5f5f5f' : '#dadada' }" class="preview">A</div>
							</button>
						</div>
					</div>
				</MkFolder>
