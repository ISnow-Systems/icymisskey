...
<template>
	<div class="_gaps">
		<div class="_gaps">
				...
				<div class="_gaps_m">
					<template v-if="instance.federation !== 'none'">
						<MkRadios v-model="hostSelect">
							...
							<option default value="all">{{ i18n.ts.all }}</option>
					...
					<MkFolder :defaultOpen="true">
						...
						<div class="_gaps">
							<div :class="$style.userItem">
								...
								<MkButton v-if="user == null && $i != null" :class="$style.addMeButton" transparent @click="selectSelf">
									<div :class="$style.addUserButtonInner"><span><i class="ti ti-plus"></i><i class="ti ti-user"></i></span><span>{{ i18n.ts.selectSelf }}</span></div>
								</MkButton>
								<MkButton v-if="user == null" :class="$style.addUserButton" transparent @click="selectUser">
									<div :class="$style.addUserButtonInner"><i class="ti ti-plus"></i><span>{{ i18n.ts.selectUser }}</span></div>
								</MkButton>
								<button :class="$style.remove" :disabled="user == null" class="_button" @click="removeUser"><i class="ti ti-x"></i></button>
							</div>
						</div>
					</MkFolder>
				</div>
			...
			<div>
				<MkButton gradate large primary rounded style="margin: 0 auto;" @click="search">{{ i18n.ts.search }}</MkButton>
			</div>
		</div>
