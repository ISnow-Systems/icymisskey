			...
			<MkSpacer :contentMax="900">
				<div class="ogwlenmc">
					<div v-if="tab === 'local'" class="local">
						<MkInput v-model="query" :debounce="true" autocapitalize="off" type="search">
						...
						<div v-if="selectMode" class="_buttons">
							...
							<MkButton danger inline @click="delBulk">Delete</MkButton>
						</div>
						<MkPagination ref="emojisPaginationComponent" :pagination="pagination">
							<template #empty><span>{{ i18n.ts.noCustomEmojis }}</span></template>
							<template #default="{items}">
								<div class="ldhfsamy">
									<button v-for="emoji in items" :key="emoji.id" :class="{ selected: selectedEmojis.includes(emoji.id) }" class="emoji _panel _button" @click="selectMode ? toggleSelect(emoji) : edit(emoji)">
										<img :alt="emoji.name" :src="emoji.url" class="img"/>
										<div class="body">
					...
					<div v-else-if="tab === 'remote'" class="remote">
						<FormSplit>
							<MkInput v-model="queryRemote" :debounce="true" autocapitalize="off" type="search">
								<template #prefix><i class="ti ti-search"></i></template>
								<template #label>{{ i18n.ts.search }}</template>
							</MkInput>
							...
							<template #default="{items}">
								<div class="ldhfsamy">
									<div v-for="emoji in items" :key="emoji.id" class="emoji _panel _button" @click="remoteMenu(emoji, $event)">
										<img :alt="emoji.name" :src="getProxiedImageUrl(emoji.url, 'emoji')" class="img"/>
										<div class="body">
											<div class="name _monospace">{{ emoji.name }}</div>
