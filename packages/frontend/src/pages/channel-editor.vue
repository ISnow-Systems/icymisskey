	...
	<MkStickyContainer>
		...
		<MkSpacer :contentMax="700">
			<div v-if="channelId == null || channel != null" class="_gaps_m">
				...
				<MkTextarea v-model="description" :mfmPreview="true" mfmAutocomplete>
						...
						>
							<template #item="{element,index}">
								<div :class="$style.pinnedNote">
									<button :class="$style.pinnedNoteHandle" class="_button"><i class="ti ti-menu"></i></button>
									{{ element.id }}
									<button :class="$style.pinnedNoteRemove" class="_button" @click="removePinnedNote(index)"><i class="ti ti-x"></i></button>
								</div>
							</template>
						</Sortable>
					</div>
