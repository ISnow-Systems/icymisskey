...
<template>
	<div class="_gaps_m">
				...
				>
					<template #item="{element,index}">
						<div
							...
							:class="$style.item"
						>
							<button :class="$style.itemHandle" class="_button"><i class="ti ti-menu"></i></button>
							<i :class="[$style.itemIcon, navbarItemDef[element.type]?.icon]" class="ti-fw"></i><span :class="$style.itemText">{{ navbarItemDef[element.type]?.title ?? i18n.ts.divider }}</span>
							<button :class="$style.itemRemove" class="_button" @click="removeItem(index)"><i class="ti ti-x"></i></button>
						</div>
					</template>
				</Sortable>
			</MkContainer>
		...
		<div class="_buttons">
			...
			<MkButton class="save" primary @click="save"><i class="ti ti-device-floppy"></i> {{ i18n.ts.save }}</MkButton>
		</div>
