		...
		<MkFolder :defaultOpen="true">
			...
			<div class="_gaps">
					...
					<div v-panel style="border-radius: 6px;">
						...
						>
							<template #item="{element}">
								<button :class="$style.emojisItem" class="_button" @click="removeReaction(element, $event)">
									<MkCustomEmoji v-if="element[0] === ':'" :fallbackToImage="true" :name="element" :normal="true"/>
									<MkEmoji v-else :emoji="element" :normal="true"/>
								</button>
							</template>
							<template #footer>
								<button :class="$style.emojisAdd" class="_button" @click="chooseReaction">
				...
				<div class="_buttons">
					...
					<MkButton danger inline @click="setDefaultReaction"><i class="ti ti-reload"></i> {{ i18n.ts.default }}</MkButton>
					<MkButton danger inline @click="overwriteFromPinnedEmojis"><i class="ti ti-copy"></i> {{ i18n.ts.overwriteFromPinnedEmojis }}</MkButton>
				</div>
			</div>
		</MkFolder>
			...
			<div class="_gaps">
				<div>
					<div v-panel style="border-radius: 6px;">
						...
						>
							<template #item="{element}">
								<button :class="$style.emojisItem" class="_button" @click="removeEmoji(element, $event)">
									<MkCustomEmoji v-if="element[0] === ':'" :fallbackToImage="true" :name="element" :normal="true"/>
									<MkEmoji v-else :emoji="element" :normal="true"/>
								</button>
							</template>
							<template #footer>
								<button :class="$style.emojisAdd" class="_button" @click="chooseEmoji">
									<i class="ti ti-plus"></i>
								</button>
							</template>
						</Sortable>
				...
				<div class="_buttons">
					...
					<MkButton danger inline @click="setDefaultEmoji"><i class="ti ti-reload"></i> {{ i18n.ts.default }}</MkButton>
					<MkButton danger inline @click="overwriteFromPinnedEmojisForReaction"><i class="ti ti-copy"></i> {{ i18n.ts.overwriteFromPinnedEmojisForReaction }}</MkButton>
				</div>
			</div>
		</MkFolder>
