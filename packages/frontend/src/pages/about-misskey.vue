			...
			<MkSpacer :contentMax="600" :marginMin="20">
				<div class="_gaps_m znqjceqz">
					<div v-panel class="about">
						<div ref="containerEl" :class="{ playing: easterEggEngine != null }" class="container">
							<img alt="" class="icon" draggable="false" src="/client-assets/about-icon.png" @click="gravity" @load="iconLoaded"/>
							<div class="misskey">IcyMisskey</div>
							<div class="version">Version: {{ version.replace(/[\d\.]+-icy\(([\d\.]+)\)/, "$1") }}</div>
							<span v-for="emoji in easterEggEmojis" :key="emoji.id" :class="{ _physics_circle_: !emoji.emoji.startsWith(':') }" :data-physics-x="emoji.left" :data-physics-y="emoji.top" class="emoji">
							<MkCustomEmoji v-if="emoji.emoji[0] === ':'" :fallbackToImage="true" :name="emoji.emoji" :noStyle="true" :normal="true" class="emoji"/>
							<MkEmoji v-else :emoji="emoji.emoji" :noStyle="true" :normal="true" class="emoji"/>
						</span>
						</div>
						<button v-if="thereIsTreasure" class="_button treasure" @click="getTreasure"><img class="treasureImg" src="/fluent-emoji/1f3c6.png"></button>
					</div>
					<div style="text-align: center;">
						{{ i18n.ts._aboutMisskey.about }}<br><a class="_link" href="https://misskey-hub.net/docs/about-misskey/" target="_blank">{{ i18n.ts.learnMore }}</a>
					</div>
					<div v-if="$i != null" style="text-align: center;">
						<MkButton inline primary rounded @click="iLoveMisskey">I
							<Mfm text="$[jelly ❤]"/>
							#Misskey
						</MkButton>
					</div>
					<FormSection>
						<div class="_gaps_s">
							<FormLink external to="https://github.com/ISnow-Systems/icymisskey">
								<template #icon><i class="ti ti-code"></i></template>
								{{ i18n.ts._aboutMisskey.source }}(IcyMisskey)
								<template #suffix>GitHub</template>
							</FormLink>
							<FormLink external to="https://kori.fanbox.cc/">
								<template #icon><i class="ti ti-pig-money"></i></template>
								{{ i18n.ts._aboutMisskey.donate }}(Subscription)
								<template #suffix>pixivFANBOX</template>
							</FormLink>
							<FormLink external to="https://www.buymeacoffee.com/kori16syusetu">
								<template #icon><i class="ti ti-pig-money"></i></template>
					...
					<FormSection>
						<div class="_gaps_s">
							<FormLink external to="https://github.com/misskey-dev/misskey">
								<template #icon><i class="ti ti-code"></i></template>
								{{ i18n.ts._aboutMisskey.source }} ({{ i18n.ts._aboutMisskey.original }})
								<template #suffix>GitHub</template>
							</FormLink>
							<FormLink external to="https://crowdin.com/project/misskey">
								<template #icon><i class="ti ti-language-hiragana"></i></template>
								{{ i18n.ts._aboutMisskey.translation }}
								<template #suffix>Crowdin</template>
							</FormLink>
							<FormLink external to="https://www.patreon.com/syuilo">
								<template #icon><i class="ti ti-pig-money"></i></template>
					...
					<FormSection>
						...
						<div :class="$style.contributors">
							<a :class="$style.contributor" href="https://github.com/syuilo" target="_blank">
								<img :class="$style.contributorAvatar" src="https://avatars.githubusercontent.com/u/4439005?v=4">
								<span :class="$style.contributorUsername">@syuilo</span>
							</a>
							<a :class="$style.contributor" href="https://github.com/acid-chicken" target="_blank">
								<img :class="$style.contributorAvatar" src="https://avatars.githubusercontent.com/u/20679825?v=4">
								<span :class="$style.contributorUsername">@acid-chicken</span>
							</a>
							<a :class="$style.contributor" href="https://github.com/kakkokari-gtyih" target="_blank">
								<img :class="$style.contributorAvatar" src="https://avatars.githubusercontent.com/u/67428053?v=4">
								<span :class="$style.contributorUsername">@kakkokari-gtyih</span>
							</a>
							<a :class="$style.contributor" href="https://github.com/tai-cha" target="_blank">
								<img :class="$style.contributorAvatar" src="https://avatars.githubusercontent.com/u/40626578?v=4">
								<span :class="$style.contributorUsername">@tai-cha</span>
							</a>
							<a :class="$style.contributor" href="https://github.com/samunohito" target="_blank">
								<img :class="$style.contributorAvatar" src="https://avatars.githubusercontent.com/u/46447427?v=4">
								<span :class="$style.contributorUsername">@samunohito</span>
							</a>
							<a :class="$style.contributor" href="https://github.com/anatawa12" target="_blank">
								<img :class="$style.contributorAvatar" src="https://avatars.githubusercontent.com/u/22656849?v=4">
					...
					<FormSection>
						...
						<div style="display:grid;grid-template-columns:repeat(auto-fill, minmax(130px, 1fr));grid-gap:24px;align-items:center;">
							<div>
								<a class="masknetwork" href="https://mask.io/" style="display: inline-block;" target="_blank" title="Mask Network"><img alt="Mask Network" src="https://assets.misskey-hub.net/sponsors/masknetwork.png" style="width: 100%;"></a>
							</div>
							<div>
								<a class="xserver" href="https://www.xserver.ne.jp/" style="display: inline-block;" target="_blank" title="XServer"><img alt="XServer" src="https://assets.misskey-hub.net/sponsors/xserver.png" style="width: 100%;"></a>
							</div>
							<div>
								<a class="skeb" href="https://skeb.jp/" style="display: inline-block;" target="_blank" title="Skeb"><img alt="Skeb" src="https://assets.misskey-hub.net/sponsors/skeb.svg" style="width: 100%;"></a>
							</div>
							<div>
								<a class="pepabo" href="https://pepabo.com/" style="display: inline-block;" target="_blank" title="GMO Pepabo"><img alt="GMO Pepabo" src="https://assets.misskey-hub.net/sponsors/gmo_pepabo.svg" style="width: 100%;"></a>
							</div>
						</div>
					</FormSection>
					<FormSection>
						...
						<div :class="$style.patronsWithIcon">
							<div v-for="patron in patronsWithIcon" :class="$style.patronWithIcon">
								<img :class="$style.patronIcon" :src="patron.icon">
