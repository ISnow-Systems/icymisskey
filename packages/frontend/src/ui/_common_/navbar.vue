	...
	<div :class="[$style.root, { [$style.iconOnly]: iconOnly }]">
		<div :class="$style.body">
			<div :class="$style.top">
				...
				<button v-tooltip.noDelay.right="instance.name ?? i18n.ts.instance" :class="$style.instance" class="_button" @click="openInstanceMenu">
					<img :class="$style.instanceIcon" :src="instance.iconUrl || instance.faviconUrl || '/favicon.ico'" alt=""/>
				</button>
			</div>
			<div :class="$style.middle">
				<MkA v-tooltip.noDelay.right="i18n.ts.timeline" :activeClass="$style.active" :class="$style.item" exact to="/">
				...
				<template v-for="item in menu">
					...
					>
						<i :class="[$style.itemIcon, navbarItemDef[item].icon]" class="ti-fw"></i><span :class="$style.itemText">{{ navbarItemDef[item].title }}</span>
						<span v-if="navbarItemDef[item].indicated" :class="$style.itemIndicator" class="_blink">
						<span v-if="navbarItemDef[item].indicateValue" :class="$style.itemIndicateValueIcon" class="_indicateCounter">{{ navbarItemDef[item].indicateValue }}</span>
						<i v-else class="_indicatorCircle"></i>
					</span>
					</component>
				</template>
				<div :class="$style.divider"></div>
				<MkA v-if="$i != null && ($i.isAdmin || $i.isModerator)" v-tooltip.noDelay.right="i18n.ts.controlPanel" :activeClass="$style.active" :class="$style.item" to="/admin">
				...
				<MkA v-tooltip.noDelay.right="i18n.ts.settings" :activeClass="$style.active" :class="$style.item" to="/settings">
					<i :class="$style.itemIcon" class="ti ti-settings ti-fw"></i><span :class="$style.itemText">{{ i18n.ts.settings }}</span>
				</MkA>
			</div>
			<div :class="$style.bottom">
				<button v-tooltip.noDelay.right="i18n.ts.note" :class="[$style.post]" class="_button" data-cy-open-post-form @click="() => { os.post(); }">
					<i :class="$style.postIcon" class="ti ti-pencil ti-fw"></i><span :class="$style.postText">{{ i18n.ts.note }}</span>
				</button>
				<button v-if="$i != null" v-tooltip.noDelay.right="`${i18n.ts.account}: @${$i.username}`" :class="[$style.account]" class="_button" @click="openAccountMenu">
					<MkAvatar :class="$style.avatar" :user="$i"/>
					<MkAcct :class="$style.acct" :user="$i" class="_nowrap"/>
				</button>
			</div>
		</div>
		<button v-if="!forceIconOnly" :class="$style.toggleButton" class="_button" @click="toggleIconOnly">
			<!--
			...
			-->
			<svg :class="$style.toggleButtonShape" viewBox="0 0 16 64">
