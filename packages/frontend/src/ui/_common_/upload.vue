...
<template>
	<div :style="{ zIndex }" class="mk-uploader _acrylic">
		<ol v-if="uploads.length > 0">
			<li v-for="ctx in uploads" :key="ctx.id">
				<div :style="{ backgroundImage: `url(${ ctx.img })` }" class="img"></div>
				<div class="top">
					<p class="name">
						<MkLoading :em="true"/>
						{{ ctx.name }}
				...
				<progress :class="{ initing: ctx.progressValue === undefined, waiting: ctx.progressValue !== undefined && ctx.progressValue === ctx.progressMax }" :max="ctx.progressMax || 0" :value="ctx.progressValue || 0"></progress>
			</li>
		</ol>
	</div>
</template>
