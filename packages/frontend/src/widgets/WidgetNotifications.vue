...
<template>
	<MkContainer :scrollable="true" :showHeader="widgetProps.showHeader" :style="`height: ${widgetProps.height}px;`" class="mkw-notifications" data-cy-mkw-notifications>
		<template #icon><i class="ti ti-bell"></i></template>
		<template #header>{{ i18n.ts.notifications }}</template>
		<template #func="{ buttonStyleClass }">
			<button :class="buttonStyleClass" class="_button" @click="configureNotification()"><i class="ti ti-settings"></i></button>
		</template>
