...
<template>
	<div>
		...
		<MkSpacer :marginMax="32" :marginMin="20">
			<form autocomplete="new-password" class="_gaps_m" @submit.prevent="onSubmit">
				<MkInput v-if="instance.disableRegistration" v-model="invitationCode" :spellcheck="false" required type="text">
					<template #label>{{ i18n.ts.invitationCode }}</template>
					<template #prefix><i class="ti ti-key"></i></template>
				</MkInput>
				<MkInput v-model="username" :spellcheck="false" autocomplete="username" data-cy-signup-username pattern="^[a-zA-Z0-9_]{1,20}$" required type="text" @update:modelValue="onChangeUsername">
				...
				<MkInput v-if="instance.emailRequiredForSignup" v-model="email" :debounce="true" :spellcheck="false" data-cy-signup-email required type="email" @update:modelValue="onChangeEmail">
					<template #label>{{ i18n.ts.emailAddress }}
						<div v-tooltip:dialog="i18n.ts._signup.emailAddressInfo" class="_button _help"><i class="ti ti-help-circle"></i></div>
					</template>
				...
				<MkInput v-model="password" autocomplete="new-password" data-cy-signup-password required type="password" @update:modelValue="onChangePassword">
				...
				<MkInput v-model="retypedPassword" autocomplete="new-password" data-cy-signup-password-retype required type="password" @update:modelValue="onChangePasswordRetype">
				...
				<MkCaptcha v-if="instance.enableHcaptcha" ref="hcaptcha" v-model="hCaptchaResponse" :class="$style.captcha" :sitekey="instance.hcaptchaSiteKey" provider="hcaptcha"/>
				<MkCaptcha v-if="instance.enableMcaptcha" ref="mcaptcha" v-model="mCaptchaResponse" :class="$style.captcha" :instanceUrl="instance.mcaptchaInstanceUrl" :sitekey="instance.mcaptchaSiteKey" provider="mcaptcha"/>
				<MkCaptcha v-if="instance.enableRecaptcha" ref="recaptcha" v-model="reCaptchaResponse" :class="$style.captcha" :sitekey="instance.recaptchaSiteKey" provider="recaptcha"/>
				<MkCaptcha v-if="instance.enableTurnstile" ref="turnstile" v-model="turnstileResponse" :class="$style.captcha" :sitekey="instance.turnstileSiteKey" provider="turnstile"/>
				...
				<MkButton :disabled="shouldDisableSubmitting" data-cy-signup-submit gradate large rounded style="margin: 0 auto;" type="submit">
