		...
		>
			<div v-if="phase === 'accountSelect'" key="accountSelect" :class="$style.root" class="_gaps">
				...
				<div>
					...
					<div :class="$style.accountSelectorList">
						<template v-for="[id, user] in users">
							<input :id="'account-' + id" v-model="selectedUser" :class="$style.accountSelectorRadio" :value="id" name="accountSelector" type="radio"/>
							<label :class="$style.accountSelectorItem" :for="'account-' + id">
								<MkAvatar :class="$style.accountSelectorAvatar" :user="user"/>
								<div :class="$style.accountSelectorBody">
									<MkUserName :class="$style.accountSelectorName" :user="user"/>
									<MkAcct :class="$style.accountSelectorAcct" :user="user"/>
								</div>
							</label>
						</template>
						<button :class="[$style.accountSelectorItem, $style.accountSelectorAddAccountRoot]" class="_button" @click="clickAddAccount">
				...
				<div class="_buttonsCenter">
					<MkButton :disabled="selectedUser === null" gradate rounded @click="clickChooseAccount">{{ i18n.ts.continue }} <i class="ti ti-arrow-right"></i></MkButton>
				</div>
			</div>
			<div v-else-if="phase === 'consent'" key="consent" :class="$style.root" class="_gaps">
				...
				<div v-if="permissions && permissions.length > 0" :class="$style.permissionRoot" class="_gaps_s">
				...
				<div>
					<div :class="$style.accountSelectorLabel">
						...
						<button class="_textButton" @click="clickBackToAccountSelect">{{ i18n.ts.switchAccount }}</button>
					</div>
					<div :class="$style.accountSelectorList">
						<div :class="[$style.accountSelectorItem, $style.static]">
							<MkAvatar :class="$style.accountSelectorAvatar" :user="users.get(selectedUser!)!"/>
							<div :class="$style.accountSelectorBody">
								<MkUserName :class="$style.accountSelectorName" :user="users.get(selectedUser!)!"/>
								<MkAcct :class="$style.accountSelectorAcct" :user="users.get(selectedUser!)!"/>
							</div>
						</div>
					</div>
				</div>
				<div class="_buttonsCenter">
					...
					<MkButton gradate rounded @click="clickAccept">{{ i18n.ts.accept }}</MkButton>
				</div>
			</div>
