...
<template>
	<div :class="$style.wrapper">
		<div :class="$style.root" class="_gaps">
			...
			<MkButton :disabled="queryingKey" large primary rounded style="margin: 0 auto;" @click="queryKey">{{ i18n.ts.retry }}</MkButton>

			<MkButton v-if="isPerformingPasswordlessLogin !== true" :disabled="queryingKey" rounded style="margin: 0 auto;" transparent @click="emit('useTotp')">{{ i18n.ts.useTotp }}</MkButton>
		</div>
	</div>
</template>
