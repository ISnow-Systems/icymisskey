...
<template>
	<MkModal ref="modal" :preferType="'dialog'" :zPriority="'high'" @click="done(true)" @closed="emit('closed')" @esc="cancel()">
		<div :class="$style.root">
			...
			<MkInput v-if="input" v-model="inputValue" :autocomplete="input.autocomplete" :placeholder="input.placeholder || undefined" :type="input.type || 'text'" autofocus @keydown="onInputKeydown">
			...
			<div v-if="(showOkButton || showCancelButton) && !actions" :class="$style.buttons">
				<MkButton v-if="showOkButton" :autofocus="!input && !select" :disabled="okButtonDisabledReason != null" data-cy-modal-dialog-ok inline primary rounded @click="ok">{{ okText ?? ((showCancelButton || input || select) ? i18n.ts.ok : i18n.ts.gotIt) }}</MkButton>
				<MkButton v-if="showCancelButton || input || select" data-cy-modal-dialog-cancel inline rounded @click="cancel">{{ cancelText ?? i18n.ts.cancel }}</MkButton>
			</div>
			<div v-if="actions" :class="$style.buttons">
				<MkButton v-for="action in actions" :key="action.text" :danger="action.danger" :primary="action.primary" inline rounded @click="() => { action.callback(); modal?.close(); }">{{ action.text }}</MkButton>
			</div>
		</div>
	</MkModal>
</template>
