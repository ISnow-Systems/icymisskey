		...
		<MkSpacer :marginMax="32" :marginMin="20">
			<div v-if="Object.keys(form).filter(item => !form[item].hidden).length > 0" class="_gaps_m">
				<template v-for="(v, k) in Object.fromEntries(Object.entries(form))">
					...
					<MkInput v-else-if="v.type === 'number'" v-model="values[k]" :step="v.step || 1" type="number">
						<template #label><span v-text="v.label || k"></span><span v-if="v.required === false"> ({{ i18n.ts.optional }})</span></template>
						<template v-if="v.description" #caption>{{ v.description }}</template>
					</MkInput>
					<MkInput v-else-if="v.type === 'string' && !v.multiline" v-model="values[k]" :mfmAutocomplete="v.treatAsMfm" type="text">
						<template #label><span v-text="v.label || k"></span><span v-if="v.required === false"> ({{ i18n.ts.optional }})</span></template>
						<template v-if="v.description" #caption>{{ v.description }}</template>
					</MkInput>
					...
					<MkRange v-else-if="v.type === 'range'" v-model="values[k]" :max="v.max" :min="v.min" :step="v.step" :textConverter="v.textConverter">
						<template #label><span v-text="v.label || k"></span><span v-if="v.required === false"> ({{ i18n.ts.optional }})</span></template>
						<template v-if="v.description" #caption>{{ v.description }}</template>
					</MkRange>
