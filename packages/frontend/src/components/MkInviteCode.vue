			...
			<div :class="$style.items">
				...
				<div v-if="moderator">
					...
					<div v-if="invite.createdBy" :class="$style.user">
						<MkAvatar :class="$style.avatar" :user="invite.createdBy" link preview/>
						<MkUserName :nowrap="false" :user="invite.createdBy"/>
				...
				<div v-if="invite.used">
					...
					<div v-if="invite.usedBy" :class="$style.user">
						<MkAvatar :class="$style.avatar" :user="invite.usedBy" link preview/>
						<MkUserName :nowrap="false" :user="invite.usedBy"/>
				...
				<div v-if="invite.expiresAt && !invite.used">
					...
					<div>
						<MkTime :time="invite.expiresAt" mode="absolute"/>
					</div>
				</div>
				<div v-if="invite.usedAt">
					<div :class="$style.label">{{ i18n.ts.inviteCodeUsedAt }}</div>
					<div>
						<MkTime :time="invite.usedAt" mode="absolute"/>
					</div>
				</div>
				<div v-if="moderator">
					<div :class="$style.label">{{ i18n.ts.createdAt }}</div>
					<div>
						<MkTime :time="invite.createdAt" mode="absolute"/>
					</div>
				</div>
			</div>
		</div>
