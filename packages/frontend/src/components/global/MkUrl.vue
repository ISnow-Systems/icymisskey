...
<template>
	<component
		:is="self ? 'MkA' : 'a'" ref="el" :[attr]="self ? props.url.substring(local.length) : props.url" :behavior="props.navigationBehavior" :class="$style.root" :rel="rel ?? 'nofollow noopener'" :target="target"
		class="_link"
		@contextmenu.stop="() => {}"
	>
		<template v-if="!self">
